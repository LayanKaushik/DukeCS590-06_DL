...
class DoubleDQNAgent:
    def __init__(self, state_dim, action_dim, epsilon, gamma, learning_rate, buffer_size, batch_size):
        ...
        self.online_model = DQN(state_dim, action_dim).to(self.device)
    ...
    def update_model(self):
        ...
        dones = dones.unsqueeze(1).to(self.device)
                ...
                # Calculate Q-values using the online model
        q_values = self.online_model(states).gather(1, actions)

        # Calculate Q-values for next states using the target model
        with torch.no_grad():
...
def main():
    ...
    agent = DoubleDQNAgent(state_dim, action_dim, epsilon, gamma, learning_rate, buffer_size, batch_size)
